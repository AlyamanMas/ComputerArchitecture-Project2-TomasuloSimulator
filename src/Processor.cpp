            ...
            for (auto &station : reservation_stations) {
                std::visit(
                    [&station, &issued, &PC, this](auto &&i) {
                        if (!station.busy && station.unit_type == i.reservation_station && !issued && areOperandsReady(i, *this)) {
                            station.busy = true;
                            auto [op1, op2] = getOperands(i);
                            station.j = op1;
                            station.k = op2;
                            station.address = PC;
                            station.cycles_counter = 0; // Initialize cycle counter for new instruction
                            // station.instr = instr; // Store a reference to the instruction
                            i.issue = true;
...
void Processor::execute(std::vector<Instruction> &instructions,
                        std::vector<ReservationStation<WordSigned, RSIndex>> &reservation_stations) {
    for (auto &station : reservation_stations) {
        if (station.busy && !get_variant_index(station.j) && !get_variant_index(station.k)) {
            std::visit(
                [&station, &instructions, this](auto &&instr) {
                    if (!instr.execute) {
                        ...
                        if (station.cycles_counter >= station.cycles_for_exec) {
                            station.busy = false;
                            std::visit([](auto &&instr) { instr.execute = true; }, instructions[station.address]);
...
void Processor::writeback(std::vector<Instruction> &instructions,
    ...
    for (auto &station : reservation_stations) {
        if (!station.busy && station.address != static_cast<Address>(-1)) {
            ...
            if (std::visit([](auto &&instr) { return instr.execute; }, instructions[station.address])) {
                ...
                if (destRegOpt.has_value()) {
                    ...
                    // Check if the register status table indicates that this station is producing the value
                    if (register_status_table[destReg] && register_status_table[destReg].value() == station.address) {
                        // Write back the result to the destination register
                        std::visit([this, destReg](auto &&instr) {
                            if constexpr (std::is_same_v<std::decay_t<decltype(instr)>, LoadInstruction> ||
                                ...
                                if (instr.result.has_value()) {
                                    ...
                                    std::cout << "Writeback: Result of instruction written to destination register R"
                                              << destReg << " with value " << instr.result.value() << std::endl;
                                }
                            }
                        }, instructions[station.address]);
                ...
                std::visit([](auto &&instr) { instr.write_result = true; }, instructions[station.address]);
            }
        }
    }
}
